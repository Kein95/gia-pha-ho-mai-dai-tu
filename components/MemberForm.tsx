...
export default function MemberForm({
  ...
  const [birthMonth, setBirthMonth] = useState<number | "">(initialData?.birth_month || "");
  ...
  const [deathMonth, setDeathMonth] = useState<number | "">(initialData?.death_month || "");
  const [deathDay, setDeathDay] = useState<number | "">(initialData?.death_day || "");
  const [deathLunarYear, setDeathLunarYear] = useState<number | "">(initialData?.death_lunar_year || "");
  const [deathLunarMonth, setDeathLunarMonth] = useState<number | "">(initialData?.death_lunar_month || "");
  const [deathLunarDay, setDeathLunarDay] = useState<number | "">(initialData?.death_lunar_day || "");

  const [isDeceased, setIsDeceased] = useState<boolean>(initialData?.is_deceased || false);
  ...
  const [currentResidence, setCurrentResidence] = useState(initialData?.current_residence || "");
  ...
  const handleSolarDeathChange = (field: "day" | "month" | "year", val: string) => {
    ...
    else { y = num; setDeathYear(num); }

    if (d !== "" && m !== "" && y !== "" && (y as number) > 100) {
      try {
        const solar = Solar.fromYmd(y as number, m as number, d as number);
  ...
  const handleLunarDeathChange = (field: "day" | "month" | "year", val: string) => {
    ...
    else { y = num; setDeathLunarYear(num); }

    if (d !== "" && m !== "" && y !== "" && (y as number) > 100) {
      try {
        const lunar = Lunar.fromYmd(y as number, m as number, d as number);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const isValidDate = (day: number | "", month: number | "", year: number | "") => {
      ...
      if (day !== "" && month !== "") {
        ...
        const daysInMonth = new Date(currentYear as number, month as number, 0).getDate();
        if ((day as number) > daysInMonth) return false;
      }
      return true;
    };
    ...
    if (isDeceased && deathLunarDay !== "" && deathLunarMonth !== "" && deathLunarYear !== "" &&
      ...
      try {
        const lunarDate = Lunar.fromYmd(deathLunarYear as number, deathLunarMonth as number, deathLunarDay as number);
      ...
      try {
        const solarDate = Solar.fromYmd(deathYear as number, deathMonth as number, deathDay as number);
    ...
    if (isDeceased && birthYear !== "" && finalDeathYear !== "" && (finalDeathYear as number) < (birthYear as number)) {
    ...
    try {
      ...
      const result = await upsertPerson({
        ...
        deathYear: isDeceased && finalDeathYear !== "" ? Number(finalDeathYear) : null,
        deathMonth: isDeceased && finalDeathMonth !== "" ? Number(finalDeathMonth) : null,
        deathDay: isDeceased && finalDeathDay !== "" ? Number(finalDeathDay) : null,
        deathLunarYear: isDeceased && finalDeathLunarYear !== "" ? Number(finalDeathLunarYear) : null,
        deathLunarMonth: isDeceased && finalDeathLunarMonth !== "" ? Number(finalDeathLunarMonth) : null,
        deathLunarDay: isDeceased && finalDeathLunarDay !== "" ? Number(finalDeathLunarDay) : null,
        isDeceased,
        isInLaw,
      ...
      if (!result.personId) throw new Error("Không lấy được ID thành viên sau khi lưu.");

      if (onSuccess) {
        onSuccess(result.personId);
      } else {
        router.push("/dashboard/members/" + result.personId);
        router.refresh();
      }
    } catch (err) {
      console.error("Error saving member:", err);
      setError((err as Error).message || "Không thể lưu hồ sơ thành viên.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="md:col-span-2">
            ...
            <div className="grid grid-cols-3 gap-3">
              <input type="number" placeholder="Ngày" min="1" max="31"
                value={birthDay} onChange={(e) => setBirthDay(e.target.value ? Number(e.target.value) : "")}
                className={inputClasses} />
              <input type="number" placeholder="Tháng" min="1" max="12"
                value={birthMonth} onChange={(e) => setBirthMonth(e.target.value ? Number(e.target.value) : "")}
                className={inputClasses} />
                ...
                <div className="relative flex items-center">
                  <input type="checkbox" checked={isDeceased}
                    onChange={(e) => {
                      ...
                      if (!e.target.checked) {
                        setDeathYear(""); setDeathMonth(""); setDeathDay("");
                        setDeathLunarYear(""); setDeathLunarMonth(""); setDeathLunarDay("");
                      }
                    }}
                    className="peer sr-only" />
                ...
                >
                  ...
                  <div className="flex flex-col gap-5">
                    <div>
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        <input type="number" placeholder="Ngày" min="1" max="31"
                          value={deathLunarDay} onChange={(e) => handleLunarDeathChange("day", e.target.value)} className={inputClasses} />
                        <input type="number" placeholder="Tháng" min="1" max="12"
                          value={deathLunarMonth} onChange={(e) => handleLunarDeathChange("month", e.target.value)} className={inputClasses} />
                        <input type="number" placeholder="Năm"
                          value={deathLunarYear} onChange={(e) => handleLunarDeathChange("year", e.target.value)} className={inputClasses} />
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        <input type="number" placeholder="Ngày" min="1" max="31"
                          value={deathDay} onChange={(e) => handleSolarDeathChange("day", e.target.value)} className={inputClasses} />
                        <input type="number" placeholder="Tháng" min="1" max="12"
                          value={deathMonth} onChange={(e) => handleSolarDeathChange("month", e.target.value)} className={inputClasses} />
                        <input type="number" placeholder="Năm"
                          value={deathYear} onChange={(e) => handleSolarDeathChange("year", e.target.value)} className={inputClasses} />
                      </div>
                    </div>
                  </div>
                </motion.div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
            <div>
              ...
              {isDeceased && (
                <p className="text-[11px] font-medium text-rose-500 mt-1.5 flex items-center gap-1">
                  <AlertCircle className="size-3" /> Không thể nhập SĐT cho người đã mất
                </p>
              )}
            </div>
            <div>
            ...
            <div className="md:col-span-2">
              ...
              <input type="text" value={currentResidence}
      ...
      >
        <button type="button" onClick={() => (onCancel ? onCancel() : router.back())} className="btn">
          Hủy bỏ
        </button>
